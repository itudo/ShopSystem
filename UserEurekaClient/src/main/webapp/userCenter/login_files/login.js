...
function check_name(){
    ...
    $.post( "http://localhost:8762/service-user/login.action",{"name":name,"user_pwd":pwd},function(data){
        if(data.code == 1){
            layer.msg('登录成功，正在跳转中...',{icon: 1,time: 2000,shade: [0.8, '#393D49']},function(){
            	window.location.href="http://localhost:8762/service-user/index.jsp";
            });
        }else{
