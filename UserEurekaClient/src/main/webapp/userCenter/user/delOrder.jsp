...
<li class="fl hover"><a href="javascript:;">订单回收站 </a></li>
</ul>
</div>
		...
		if(data.code==1) {
			...
			if(data.rows.length>0) {
				for(var i=0;i<data.rows.length;i++) {
					...
					str += "<a href='javascript:changeOrder("+obj.order_id+");' class='payment'>还原订单</a>";
...
function changeOrder(id) {
	$.getJSON("http://localhost:8762/service-user/changeOrder.action",{"status":"还原","order_id":id},function(data) {
		if(data.code==1) {
			alert("订单已还原");
			showAllOrder(6);
		}
