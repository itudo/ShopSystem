...
$(function() {
	...
	$.getJSON("http://localhost:8762/service-user/getOrderDetailFromCart.action",{id_str:id},function(data) {
		if(data.code==1) {
			...
			for(var i=0;i<cart.length;i++) {
				count += cart[0].goods_count;
				price += cart[i].goods_money;
				s += "<div class='paymentCon clearfix'><div class='con-parent clearfix fl'><a href='javascript:;' class='fl position-parent'> <img src='http://localhost:8762/service-admin/imgGoods/"+cart[i].goodsDetail.images[0].image_path+"'>";
				s += "</a><div class='fl ition'><a href='javascript:;'>"+cart[i].goodsDetail.goods.goods_name+"</a></div></div><div class='fl atte'>"+cart[i].goodsDetail.attr_name+"</div>";
				s += "<div class='fl price'>"+cart[0].goodsDetail.goodsdetail_price+"</div><div class='fl number'>"+cart[0].goods_count+"</div><div class='fl sual' id='sslh'>"+cart[i].goods_money+"</div></div>";
			}
...
<input type="hidden" id="goodsdetail_id" name="id_str" value="">
...
<input type="button" onclick="InterAddress.submitOrder2()" value="提交订单" class="fr"> <a href="" class="fr">返回购物车</a>
