...
public class CollectController {
	...
	@ResponseBody
	public JsonModel setCollectGoods(Integer goods_id, HttpSession session) {
		Users users = (Users) session.getAttribute("user");
		JsonModel jsonModel = new JsonModel();
		if (users != null && !users.equals("")) {
			redisService.sAdd(users.getUser_name(), goods_id);
			jsonModel.setCode(1);
		}
		return jsonModel;
	}
	...
	@ResponseBody
	public JsonModel getCollectGoods(HttpSession session) {
		...
		if (users != null && !users.equals("")) {
			Set<Integer> set = redisService.sGet(users.getUser_name());
