...
public class UsersController {
	...
	public JsonModel login(Users user,HttpSession session) {
		...
		if(user1!=null ) {
			...
			jsonModel.setCode(1);
			usersBiz.updateLoginDate(user3);
	...
	@ResponseBody
	public JsonModel sendEmial(@RequestParam(name = "email")String email,HttpSession session) {
		JsonModel jsonModel = new JsonModel();
		int code = (int)((Math.random()*9+1)*100000);
		session.setAttribute("emailCode", code);
		boolean a = myRemote.sendEmail(code,email);
	...
	public JsonModel getAdress(HttpSession session) {
		Users user = (Users) session.getAttribute("user");
	...
	public JsonModel addAddress(Address address,HttpSession session) {
		JsonModel jsonModel = new JsonModel();
		Users user = (Users) session.getAttribute("user");
	...
	public JsonModel setDefaultAddress(Address address,HttpSession session) {
		JsonModel jsonModel = new JsonModel();
		Users user = (Users) session.getAttribute("user");
		address.setUser(user);
		addressBiz.setDefault(address);
    ...
    public  JsonModel uploadHead(HttpSession session,@RequestParam("file") MultipartFile file, HttpServletRequest request) {
    	JsonModel jsonModel = new JsonModel();
