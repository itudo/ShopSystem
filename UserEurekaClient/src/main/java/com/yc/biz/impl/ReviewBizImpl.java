...
public class ReviewBizImpl implements ReviewBiz {
	...
	public boolean insertReview(Review review) {
		...
		List<Integer> list = orderBiz.selectOrderDetailStatus(review.getOrderDetail().getOrders());
		if(!list.contains(0)){
			review.getOrderDetail().getOrders().setOrder_status(4);
			orderBiz.updateOrderstatus(review.getOrderDetail().getOrders());
		}
		return true;
	}
