...
public class GoodsBizImpl implements GoodsBiz {
	...
	public PageBean<Goods> findGoodsByCondition(Map<String, Object> map) {
		...
		if(map.get("pages")!=null && !map.get("pages").equals("")){
			pageBean.setPages(Integer.parseInt(map.get("pages").toString()));
		}
		if(map.get("pageSize")!=null && !map.get("pages").equals("") ){
	...
	@Override
	public boolean delSingleGoods(Integer goodsid) {
		int result = BaseDao.del(Goods.class, goodsid + "", "delSingleGoods");
		if (result > 0) {
			return true;
		}
		return false;
